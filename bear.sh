...
error_model= #path of the file of model error
output_dir="/proj/classif_metag_benchmark/NOBACKUP/bear/"

for file_path in /proj/classif_metag_benchmark/data/*.fna
do

    file_name=$(basename ${file_path%.fna})
    #First step: Parametric abundance profile generation,
    #this step generate abundance values from power functions that can
    #correspond to metagenomic communities.
    #Need multi-Fasta file of genome.
    #Bear support low, medium, high complexity communities derived from power
    #functions.

    parametric_abundance.pl "$file_path" high "$output_dir/abundance_$file_name"
    ...
    #read_max= Maximum number of reads to process from each bin of prefix
    #identical reads
    ...
    # TODO: replace read_min, num_max, nr and read_max by values
    drisee.py --percent -n read_min -b num_max -x read_max -t fastq -s nr \
        $error_model "$output_dir/error_rate_file_$file_name"

    #Generation of the quality score models for erroneous nucleotides
    #This command generate two file :
    # - a quality model file which contains quality scores in place of error
    # rates (.err.qual extension)
    # - a matrix file which contain two 4*4 matrice (.err.matr extension)
    #model error=  multi fastq file of WGS read.
    #error_rate_file.uc= file generated by drisee.py command

    error_quality.pl $error_model "$output_dir/error_rate_file_$file_name.uc"
    ...
    # TODO: replace total_reads, longest_read
    generate_reads.py -r "$file_path" -a "$output_dir/abundance_$file_name" \
        -o "$output_dir/reads_$file_name" -t total_reads -l longest_read
    ...
    trim_reads.py -i $error_model -f "$file_path" \
        -o "$output_dir/simulation_$file_name"
        -r "$output_dir/error_rate_file_$file_name.per" \
        -q "$error_model.err.qual" \
        -m "$error_model.err.matr"
done
